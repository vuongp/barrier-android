...
class BarrierClient(
    ...
    private val initialScreenWidth : Int,
    private val initialScreenHeight : Int
) {
    ...
    suspend fun connect(hostname: String, port: Int = 24800) {
        ...
        while (!socket.isClosed) {
            ...
            when (readPacket) {
                is Message.MouseDown -> {
                    eventListeners.forEach {
                        it.invoke(Event.MouseDown(readPacket.buttonId))
                    }
                }
                is Message.MouseMove -> {
                    eventListeners.forEach {
                ...
                is Message.MouseUp -> {
                    eventListeners.forEach {
                        it.invoke(Event.MouseUp(readPacket.buttonId))
                    }
                }
                else -> {}
            }
